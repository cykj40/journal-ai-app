...
USING (auth_user_id = current_user);
...
WITH CHECK (user_id IN (SELECT id FROM users WHERE auth_user_id = current_user));

CREATE POLICY "Users can view their own entries" ON journal_entries FOR SELECT
USING (user_id IN (SELECT id FROM users WHERE auth_user_id = current_user));
